...
class MCTSAgent(Agent):
    def __init__(self, faction: Faction, mcts_type: str, reward_function: str, rollout_no: int):
        ...
        LOGGER.info("MCTSAgent:__init__: type {}, reward_function {}, rollout_no {}"
                    .format(mcts_type, reward_function, rollout_no))

    def choose_action(self, state: list, actions: list[Action]) -> Action:
        match self.mcts_type:
            case "one-depth":
                mcts = MCTSOneDepth(state, actions, self.reward_function, self.rollout_no)
            case _:
